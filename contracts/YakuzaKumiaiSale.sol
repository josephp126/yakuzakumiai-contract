...
contract YakuzaKumiaiSale is AccessControl {
    ...
    uint256 public constant totalSales = 8930;
    uint256 public constant preSale1Count = 1024;
    uint256 public constant preSale2Count = 30;
    ...
    uint256 public constant maxPublicSaleMintPerWallet = 25;

    uint256 public constant preSale1Start = 1648954800; // 4.3, 12 pm JST
    uint256 public constant preSale1End = 1649127600; // 4.5, 12 pm JST
    uint256 public constant preSale2Start = 1649170800; // 4.6, 12 am JST
    uint256 public constant preSale2End = 1649214000; // 4.6, 12 pm JST
    uint256 public constant preSale3Start = 1649257200; // 4.7, 12 am JST
    uint256 public constant preSale3End = 1649390400; // 4.8, 12 pm JST
    uint256 public constant publicSaleStart = 1649516400; // 4.10 12 am JST

    uint256 public constant preSale1MintFee = 0.01 ether;
    uint256 public constant preSale2MintFee = 0.02 ether;
    uint256 public constant publicSaleMintFee = 0.07 ether;

    // roles
    bytes32 public constant WITHDRAWER_ROLE = keccak256("WITHDRAWER");
    bytes32 public constant ADMIN = keccak256("ADMIN");
    bytes32 public merkleRootWL1 =
        0xb5d235c7dc95984c578f696a365fc833e64459b72a1c5304d1c105ec6822d59b;
    bytes32 public merkleRootWL2 =
        0xb5d235c7dc95984c578f696a365fc833e64459b72a1c5304d1c105ec6822d59b;

    address payable public wallet1 =
        payable(0x192F6CCD0b9bd54bdA1A7e3776b718F908028EC4); // wallet address for accounts payable and marketing
    address payable public wallet2 =
        payable(0x8d6AE8FE3A583A0B60cF7670332726b9bb30A507); // wallet address for giveaway
    address payable public wallet3 =
        payable(0xd5a5c1B25EdB13ED2737247B0a7E0578705bEBe9); // wallet address for tool development
    address payable public wallet4 =
        payable(0x8C06E7617575576a26954779fbB58a467faa6f16); // wallet address for anime studio
    address payable public wallet5 =
        payable(0x80c1c51dD714d42A93E70e4544007984079EBE0B); // wallet address for dev team
    address payable public wallet6 =
        payable(0x7b62AC97F6a9Fb98Ec7F48EAA109014C1B685A95); // wallet address for addy (lead developer)
    ...
    function updateWalletAddress(string memory _walletName, address _address)
        external
        onlyAdmin
    {
        if (
            keccak256(abi.encodePacked(_walletName)) ==
            keccak256(abi.encodePacked("wallet1"))
        ) {
            wallet1 = payable(_address);
        }
        if (
            keccak256(abi.encodePacked(_walletName)) ==
            keccak256(abi.encodePacked("wallet2"))
        ) {
            wallet2 = payable(_address);
        }
        if (
            keccak256(abi.encodePacked(_walletName)) ==
            keccak256(abi.encodePacked("wallet3"))
        ) {
            wallet3 = payable(_address);
        }
        if (
            keccak256(abi.encodePacked(_walletName)) ==
            keccak256(abi.encodePacked("wallet4"))
        ) {
            wallet4 = payable(_address);
        }
        if (
            keccak256(abi.encodePacked(_walletName)) ==
            keccak256(abi.encodePacked("wallet5"))
        ) {
    ...
    function getWalletList(string memory _walletName)
        public
        view
        onlyAdmin
        returns (address)
    ...
    function isPresale2Finished() public view returns (bool) {
        return
            block.timestamp > preSale2End ||
            totalMints >= preSale1Count + preSale2Count;
    }

    /// @notice Check if presale is finished
    function isPresale3Finished() public view returns (bool) {
        return
            block.timestamp > preSale3End ||
            totalMints >= preSale1Count + preSale2Count;
    }
    ...
    function mint(address _to, uint256 _count) external payable {
        require(
            ...
            "PS: Public sale is not started"
        );
        require(
            msg.value >= publicSaleMintFee * _count,
            "PS: Not enough funds sent"
        );
        require(
            mintedTokens[_to] + _count <= maxPublicSaleMintPerWallet,
            "PS: Max limited per wallet"
        );
        _mint(_to, _count);
    }
    ...
    function whiteListMint1(
        ...
        require(
            block.timestamp >= preSale1Start,
            "PS: Presale1 is not started yet"
        );
        require(!isPresale1Finished(), "PS: Presale1 Finished");
        require(
            mintedTokens[_to] + _count <= maxPreSale1MintPerWallet,
            "PS: Max limited per wallet"
        );
        require(
            msg.value >= preSale1MintFee * _count,
            "PS: Not enough funds sent"
        );
        bytes32 leaf = keccak256(abi.encodePacked(_to));
        require(
            MerkleProof.verify(_merkleProof, merkleRootWL1, leaf),
            "PS: Failed to verify WhiteList"
        );
        _mint(_to, _count);
    }
    ...
    function whiteListMint2(
        ...
        require(
            block.timestamp >= preSale2Start,
            "PS: Presale2 is not started yet"
        );
        require(!isPresale2Finished(), "PS: Presale2 Finished");
        require(
            mintedTokens[_to] + _count <= maxPreSale2MintPerWallet,
            "PS: Max limited per wallet"
        );
        require(
            msg.value >= preSale2MintFee * _count,
            "PS: Not enough funds sent"
        );
        bytes32 leaf = keccak256(abi.encodePacked(_to));
        require(
            MerkleProof.verify(_merkleProof, merkleRootWL2, leaf),
            "PS: Failed to verify WhiteList"
        );
        _mint(_to, _count);
    }
    ...
    function whiteListMint3(
        ...
        require(
            block.timestamp >= preSale3Start,
            "PS: Presale3 is not started yet"
        );
        require(!isPresale3Finished(), "PS: Presale3 Finished");
        require(
            mintedTokens[_to] + _count <= maxPreSale3MintPerWallet,
            "PS: Max limited per wallet"
        );
        require(
            msg.value >= preSale2MintFee * _count,
    ...
    function withdrawToMarketingFunder() public payable onlyAdmin {
        ...
        require(
            ...
            "PS: Marketing funds has already been finished"
        );
        uint256 total = address(this).balance;
